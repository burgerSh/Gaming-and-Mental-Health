...
with st.sidebar:
    ...
    st.write("📧 **ติดต่อ:** [67160365@go.buu.ac.th]")
    st.markdown("---")
...
with tab1:
    ...
    with col2:
        ...
        hours = st.number_input("เล่นเกมกี่ชั่วโมง/วัน?", min_value=1.0, max_value=24.0, value=3.0, step=1.0)
    ...
    if st.button("🚀 คลิกเพื่อประเมินความเสี่ยง"):
        ...
        # 1. ให้ AI ทำนายผลก่อนตามปกติ
        prediction = rf_model.predict(input_encoded)[0]

        # ==============================================================
        # 2. กฎพิเศษ (Rule-based Override): 
        # ถ้ามีการติ๊กถูกที่อาการใดอาการหนึ่ง (True) บังคับให้เป็น "Severe" ทันที
        # ==============================================================
        if withdrawal or loss_interest or eye_strain or back_pain:
            prediction = "Severe"
